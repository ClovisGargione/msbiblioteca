...
import io.github.clovisgargione.msautenticacao.infra.repository.AuthorizationRepository;
...
public class JpaOAuth2AuthorizationService implements OAuth2AuthorizationService {
    ...
    public Authorization findByUsernameAndGrantType(String username, String grantType) {
	Assert.notNull(username, "username cannot be empty");
	return this.authorizationRepository.findByUsernameAndGrantType(username, grantType).orElse(null);
    }

    public Authorization findByToken(String token) {
	Assert.notNull(token, "token cannot be empty");
	return this.authorizationRepository.findByAccessTokenValue(token).orElse(null);
    }
	...
	entity.setAccessTokenType(OAuth2AccessToken.TokenType.BEARER.getValue());
	Authentication authentication = authorization.getAttribute(Principal.class.getName());
	if (nonNull(authentication)) {
	    if (authentication.getPrincipal() instanceof User) {
		entity.setUsername(((User) authentication.getPrincipal()).getUsername());
	    } else {
		entity.setUsername(((ResourceOwner) authentication.getPrincipal()).getUsername());
	    }
	}
